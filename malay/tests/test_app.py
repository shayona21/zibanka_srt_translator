...
import app as malay_app


class AppValidationTests(unittest.TestCase):
    def setUp(self):
        malay_app.app.config.update(TESTING=True)
        self.client = malay_app.app.test_client()

    def test_home_page_is_malay_specific(self):
        response = self.client.get("/")
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"Malaysian Malay", response.data)
