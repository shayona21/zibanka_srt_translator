...
MALAY_HEADER = "MALAY TRANSLATION"
...
Malaysian Malay (Bahasa Melayu Malaysia). Translate dialogue naturally and
conversationally for a Malaysian audience. Use the supplied character and story
...
def build_translation_prompt(batch, supporting_context, repair_hint=""):
    ...
    return f"""
Translate the rows in <rows_to_translate> from English into Malaysian Malay.

TRANSLATION GUIDELINES
1. Use Malaysian Malay (Bahasa Melayu Malaysia), not Indonesian.
...
8. Output only row_id and malay_translation. Do not include English text or commentary.
{repair_section}
<series_synopsis>
...
def _response_schema(row_count):
    return {
        ...
        "items": {
            ...
            "propertyOrdering": ["row_id", "malay_translation"],
            "properties": {
                "row_id": {"type": "integer"},
                "malay_translation": {"type": "string"},
            },
            "required": ["row_id", "malay_translation"],
        },
    }
...
def normalize_batch_result(batch_result, batch_number, source_records):
    ...
    for output_position, row in enumerate(batch_result, start=1):
        ...
        translation = row.get("malay_translation")
...
def write_translated_workbook(workbook, plans, translations, output_path):
    ...
    for plan in plans:
        ...
        target_header.value = MALAY_HEADER
...
def get_output_file_name(file_path, requested_name=None):
    ...
    return f"{safe_stem}_Malay.xlsx"
...
def process_excel_file(
    ...
    try:
        ...
        emit_progress(
            ...
            total_batches,
            total_batches,
            "Writing Malay translations into the original workbook.",
        )
